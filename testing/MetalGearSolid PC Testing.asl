...
startup {
  ...
  // Returns the current game's target FPS (30)
  F.FramesPerSecond = (Func<int>)(() => 30);
  ...
  F.AddSetting(F.SettingParent("Splits", null), true, "Split Points");
    ...
    F.AddSetting("CP-238", true, "Retrieve PAL Key (instant)");
  ...
  if (!V.InitInitiated) {
    ...
    F.UpdateASLPalKey = (Action)(() => {
      ...
      if (settings["Opt.ASL.Info.PalKey"]) {
        ...
        bool inBlastFurnace = ((loc == "s13a") && (prog == 244 || prog == 242)); // 242 it's maybe due to skipping codec calls on PC port, therefore check with JN's.
          ...
          if (validItem) {
            ...
            if (status == 1) {
              ...
              if (dir == 0) {
                ...
                double remaining = (Math.Abs(target - temp) / ratePerSecond) + 1.0;
...
";
    ...
    // Codec skip glitch: enters Command Room at CP-223 instead of CP-ReachCommandRoom
    F.Check.Add("OL-s16c.CL-s16d.CP-223", (Func<bool>)(() => {
      if (F.SettingEnabled("OL-s16c.CL-s16d.CP-ReachCommandRoom")) {
        if (!R.CompletedSplits.ContainsKey("OL-s16c.CL-s16d.CP-ReachCommandRoom"))
          R.CompletedSplits.Add("OL-s16c.CL-s16d.CP-ReachCommandRoom", true);
        F.ManualSplit("Splitting for OL-s16c.CL-s16d.CP-ReachCommandRoom (Underground Base 3)");
      }
      return false;
    }));    
    ...
    F.Watch.Add("W.CP-129", (Func<int>)(() => F.BossHealthSimple("Mantis", M["BossMaxHP"].Current)));
    ...
    F.Watch.Add("W.CP-211", (Func<int>)(() => F.BossHealthSimple("Raven", M["BossMaxHP"].Current)));
  ...
  if (G.VRMissions) {
    ...
    MM.Clear();
    MM.Add("WeaponData", New.ByteArray(F.Addr(0x38E800), 50));
    MM.Add("ItemData", New.ByteArray(F.Addr(0x38E82A), 80));
...
split {
  ...
  if ( (Math.Abs(M["GameTime"].Current - M["GameTime"].Old)) > (5 * 30) ) {
    G.FpsLog.Clear();
    return false;
  }
